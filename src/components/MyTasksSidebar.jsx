...
function MyTasksSidebar() {
    ...
    const fetchUserTasks = () => {
        ...
        const tasks = (currentWorkspace.books || []).flatMap((book) => {
            ...
            return bookTasks
                .filter((t) =>
                    ...
                    t?.assignee?.user_id === userId
                )
                .map((t) => ({ ...t, bookId: t.bookId || t.book_id || book.id }));
        });
        setMyTasks(tasks || []);
    }
        ...
        <div className="mt-6 px-3">
            <div onClick={toggleMyTasks} className="flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-zinc-800" >
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-medium text-gray-700 dark:text-zinc-300">My Manuscripts / Book Drafts</h3>
                    ...
                    <div className="space-y-1">
                        {myTasks.length === 0 ? (
                            <div className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-500 text-center">
                                No manuscripts/book drafts assigned
                            </div>
                        ) : (
                            myTasks.map((task) => (
                            <Link key={task.id} to={`/taskDetails?bookId=${task.bookId}&taskId=${task.id}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
