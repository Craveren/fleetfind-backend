...
const BookOverview = () => {
    ...
    useEffect(() => {
        setBooks(currentWorkspace?.books || []);
    }, [currentWorkspace]);
    ...
    return currentWorkspace && (
        <div className="bg-white dark:bg-zinc-950 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-200 rounded-lg overflow-hidden">
            <div className="border-b border-zinc-200 dark:border-zinc-800 p-4 flex items-center justify-between">
                <h2 className="text-md text-zinc-800 dark:text-zinc-300">Book Overview</h2>
                <Link to={'/books'} className="text-sm text-zinc-600 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-300 flex items-center">
            ...
            <div className="p-0">
                {books.length === 0 ? (
                    <div className="p-12 text-center">
                        ...
                        <p className="text-zinc-600 dark:text-zinc-400">No books yet</p>
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {books.slice(0, 5).map((book) => (
                            <Link key={book.id} to={`/authorBooksDetail?id=${book.id}&tab=publishingSteps`} className="block p-6 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-2 ml-4">
                                        <span className={`text-xs px-2 py-1 rounded ${statusColors[book.status]}`}>
                                            {book.status.replace('_', ' ').replaceAll(/\b\w/g, c => c.toUpperCase())}
                                        </span>
                                        <div className={`w-2 h-2 rounded-full border-2 ${priorityColors[book.priority]}`} />
                                    </div>
                                </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        {book.end_date && (
                                            <div className="flex items-center gap-1">
                                                ...
                                                {format(new Date(book.end_date), "MMM d, yyyy")}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between text-xs">
                                        ...
                                        <span className="text-zinc-600 dark:text-zinc-400">{book.progress || 0}%</span>
                                    </div>
                                    <div className="w-full bg-zinc-200 dark:bg-zinc-800 rounded h-1.5">
                                        <div className="h-1.5 bg-blue-500 rounded" style={{ width: `${book.progress || 0}%` }} />
                                    </div>
                                </div>
                            </Link>
                        ))}
