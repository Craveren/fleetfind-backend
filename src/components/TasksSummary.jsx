...
export default function TasksSummary() {
    ...
    const [expanded, setExpanded] = useState({}); // per-card expand state
    const navigate = useNavigate();

    // Get all tasks for all projects in current workspace
    useEffect(() => {
        if (currentWorkspace) {
            setTasks(currentWorkspace.books?.flatMap((book) => book.tasks || []) || []);
        }
    }, [currentWorkspace]);
                    ...
                    <div className="p-4">
                        {card.items.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {(expanded[card.title] ? (card.title === 'My Publishing Steps' ? myTasks : card.title === 'Overdue' ? overdueTasks : inProgressIssues) : card.items).map((issue) => (
                                    <div
                                        key={issue.id}
                                        onClick={() => navigate(`/taskDetails?bookId=${issue.bookId || issue.book_id || ''}&taskId=${issue.id}`)}
                                ...
                                {card.count > 3 && (
                                    <button
                                        onClick={() => setExpanded((prev) => ({ ...prev, [card.title]: !prev[card.title] }))}
                                    ...
                                    >
                                        {expanded[card.title] ? 'Show less' : `View ${card.count - 3} more`} <ArrowRight className="w-3 h-3 ml-2" />
                                    </button>
                                )}
                            </div>
                        )}
