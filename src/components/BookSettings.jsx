...
import AddBookMember from "./AddBookMember";
import { useDispatch } from "react-redux";
import { updateAuthorBook } from "../features/workspaceSlice";
import ErrorBoundary from "./ErrorBoundary";

export default function BookSettings({ book }) {
    if (!book) {
        return <div className="p-4 text-center text-zinc-500 dark:text-zinc-400">No book selected to edit settings.</div>;
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            dispatch(updateAuthorBook({ ...book, ...updatedFormData }));
            console.log("Book updated successfully:", updatedFormData);
        } catch (error) {
    ...
    useEffect(() => {
        if (book) {
            setFormData({
                ...book,
                start_date: new Date(book.start_date).toISOString().split('T')[0],
                end_date: new Date(book.end_date).toISOString().split('T')[0],
            });
            console.log("BookSettings - book prop received:", book);
            console.log("BookSettings - formData after setting:", formData);
        }
    }, [book]);
    ...
    return (
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className={cardClasses}>
                <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-300 mb-4">Book Details</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Name */}
                    <div className="space-y-2">
                        <label className={labelClasses}>Book Name</label>
                    ...
                    {/* Publishing Type */}
                    <div className="space-y-2">
                        <label className={labelClasses}>Publishing Type</label>
                ...
                <div className={cardClasses}>
                    <div className="flex items-center justify-between gap-4">
                        <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-300 mb-4">
                            Team Members <span className="text-sm text-zinc-600 dark:text-zinc-400">({book.members?.length || 0})</span>
                        </h2>
                        ...
                        <ErrorBoundary>
                            <AddBookMember isDialogOpen={isDialogOpen} setIsDialogOpen={setIsDialogOpen} book={book} />
                        </ErrorBoundary>
                    </div>
                        ...
                        <div className="space-y-2 mt-2 max-h-32 overflow-y-auto">
                            {book.members.map((member, index) => (
                                <div key={index} className="flex items-center justify-between px-3 py-2 rounded dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-300" >
                                    ...
                                    {book.team_lead === member.user.id && <span className="px-2 py-0.5 rounded-xs ring ring-zinc-200 dark:ring-zinc-600">Team Lead</span>}
                                </div>
                            ))}
                        </div>
                    )}
