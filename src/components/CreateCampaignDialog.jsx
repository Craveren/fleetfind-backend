...
import { X, CalendarDays, BookOpen, Tag, DollarSign, Plus } from 'lucide-react';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';
import { useDispatch, useSelector } from 'react-redux';
import { addCampaign } from '../features/campaignsSlice';
import { selectAllBooks } from '../features/workspaceSlice'; // Import the new selector
import toast from 'react-hot-toast';

const platforms = ['Facebook', 'TikTok', 'Instagram', 'Amazon', 'Takealot'];

const CreateCampaignDialog = ({ isOpen, onClose }) => {
  const dispatch = useDispatch();
  const { currentWorkspace } = useSelector((state) => state.workspace);
  const allBooks = useSelector(selectAllBooks); // Use the memoized selector
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const campaignData = {
        ...
        status: 'Upcoming', // Default status for new campaigns
        platforms: selectedPlatforms,
        budget_zar: budget ? parseFloat(budget) : 0,
        books: selectedBooks, // Array of book IDs
        performance_stats: {}, // Initialize empty stats
      };

      await dispatch(addCampaign(campaignData)).unwrap();
      toast.dismiss();
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-2xl text-zinc-900 dark:text-zinc-200">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Plus className="size-5" /> Create New Marketing Campaign
          </h2>
              ...
              {platforms.map(platform => (
                <label key={platform} className="inline-flex items-center">
                  <input
                    ...
                    className="focus:outline-none focus:ring-0 align-middle"
                  />
