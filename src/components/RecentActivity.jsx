...
const RecentActivity = () => {
    ...
    const getTasksFromCurrentWorkspace = () => {
        ...
        const fetchedTasks = currentWorkspace.books?.flatMap((book) => book.tasks?.map((task) => task) || []);
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {tasks.map((task) => {
                            ...
                            return (
                                <div
                                    key={task.id}
                                    onClick={() => navigate(`/taskDetails?bookId=${task.bookId || task.book_id || ''}&taskId=${task.id}`)}
                                    className="p-6 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors cursor-pointer"
                                >
