...
const AddBookMember = ({ isDialogOpen, setIsDialogOpen }) => {
    ...
    const book = currentWorkspace?.books?.find((p) => p.id === id);
    const bookMembersEmails = book?.members?.map((member) => member.user?.email) || [];
    ...
    if (status === 'loading' || !currentWorkspace || !book) {
      return (
        <div className="fixed inset-0 bg-black/20 dark:bg-black/50 backdrop-blur flex items-center justify-center z-50">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md text-zinc-900 dark:text-zinc-200">
            <div className="text-center py-4 text-gray-500 dark:text-zinc-400">Loading members or book data...</div>
          </div>
        </div>
      );
    }
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md text-zinc-900 dark:text-zinc-200">
                ...
                <div className="mb-4">
                    <h2 className="text-xl font-bold flex items-center gap-2">
                        <UserPlus className="size-5 text-zinc-900 dark:text-zinc-200" /> Add Member to Book
                    </h2>
                    {currentWorkspace && (
                        <p className="text-sm text-zinc-700 dark:text-zinc-400">
                            Adding to Book: <span className="text-blue-600 dark:text-blue-400">{book?.name}</span>
                        </p>
                    )}
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            {/* List All non book members from current workspace */}
                            <select value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10 mt-1 w-full rounded border border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-200 text-sm placeholder-zinc-400 dark:placeholder-zinc-500 py-2 focus:outline-none focus:border-blue-500" required >
                                ...
                                {currentWorkspace?.members
                                    .filter((member) => member.user?.email && !bookMembersEmails.includes(member.user.email))
                                    .map((member) => (
