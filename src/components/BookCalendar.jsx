...
const BookCalendar = ({ publishingSteps }) => {
    const [selectedDate, setSelectedDate] = useState(new Date());
    const [currentMonth, setCurrentMonth] = useState(new Date());

    const today = new Date();
    const getPublishingStepsForDate = (date) => publishingSteps.filter((step) => isSameDay(step.due_date, date));

    const upcomingPublishingSteps = publishingSteps
        .filter((step) => step.due_date && !isBefore(step.due_date, today) && step.status !== "DONE")
        .sort((a, b) => new Date(a.due_date) - new Date(b.due_date))
        .slice(0, 5);

    const overduePublishingSteps = publishingSteps.filter((step) => step.due_date && isBefore(step.due_date, today) && step.status !== "DONE");
    ...
    return (
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 ">
                <div className="not-dark:bg-white dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-zinc-900 dark:text-white text-md flex gap-2 items-center max-sm:hidden">
                            <CalendarIcon className="size-5" /> Publishing Steps Calendar
                        </h2>
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        {daysInMonth.map((day) => {
                            const dayPublishingSteps = getPublishingStepsForDate(day);
                            const isSelected = isSameDay(day, selectedDate);
                            const hasOverdue = dayPublishingSteps.some((s) => s.status !== "DONE" && isBefore(s.due_date, today));
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {dayPublishingSteps.length > 0 && (
                                        <span className="text-[10px] text-blue-700 dark:text-blue-400">{dayPublishingSteps.length} publishing steps</span>
                                    )}
                                </button>
                            );
                        })}
                ...
                {getPublishingStepsForDate(selectedDate).length > 0 && (
                    <div className=" not-dark:bg-white mt-6 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                        <h3 className="text-zinc-900 dark:text-white text-lg mb-3">
                            Publishing Steps for {format(selectedDate, "MMM d, yyyy")}
                        </h3>
                        <div className="space-y-3">
                            {getPublishingStepsForDate(selectedDate).map((step) => (
                                <div
                                    key={step.id}
                                    className={`bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition p-4 rounded border-l-4 ${priorityBorders[step.priority]}`}
                                >
                                    <div className="flex justify-between mb-2">
                                        <h4 className="text-zinc-900 dark:text-white font-medium">{step.title}</h4>
                                        <span className={`px-2 py-0.5 rounded text-xs ${typeColors[step.type]}`}>
                                            {step.type}
                                        </span>
                                    </div>
                                    <div className="flex justify-between text-xs text-zinc-600 dark:text-zinc-400">
                                        <span className="capitalize">{step.priority.toLowerCase()} priority</span>
                                        {step.assignee && (
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-zinc-950 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                    <h3 className="text-zinc-900 dark:text-white text-sm flex items-center gap-2 mb-3">
                        <Clock className="w-4 h-4" /> Upcoming Publishing Steps
                    </h3>
                    {upcomingPublishingSteps.length === 0 ? (
                        <p className="text-zinc-500 dark:text-zinc-400 text-sm text-center">No upcoming publishing steps</p>
                    ) : (
                        <div className="space-y-2">
                            {upcomingPublishingSteps.map((step) => (
                                <div
                                    key={step.id}
                                    className="bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 p-3 rounded-lg transition"
                                >
                                    <div className="flex justify-between items-start text-sm">
                                        <span className="text-zinc-900 dark:text-white">{step.title}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded ${typeColors[step.type]}`}>
                                            {step.type}
                                        </span>
                                    </div>
                                    <p className="text-xs text-zinc-600 dark:text-zinc-400">{format(step.due_date, "MMM d")}</p>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                {/* Overdue Publishing Steps */}
                {overduePublishingSteps.length > 0 && (
                    <div className="bg-white dark:bg-zinc-950  border border-red-300 dark:border-red-500 border-l-4 rounded-lg p-4">
                        <h3 className="text-red-700 dark:text-red-400 text-sm flex items-center gap-2 mb-3">
                            <Clock className="w-4 h-4" /> Overdue Publishing Steps ({overduePublishingSteps.length})
                        </h3>
                        <div className="space-y-2">
                            {overduePublishingSteps.slice(0, 5).map((step) => (
                                <div key={step.id} className="bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 p-3 rounded-lg transition" >
                                    ...
                                    <p className="text-xs text-red-600 dark:text-red-300">
                                        Due {format(step.due_date, "MMM d")}
                                    </p>
                                </div>
                            ))}
                            {overduePublishingSteps.length > 5 && (
                                <p className="text-xs text-zinc-500 dark:text-zinc-400 text-center">
                                    +{overduePublishingSteps.length - 5} more
                                </p>
                            )}
                        </div>
                    </div>
